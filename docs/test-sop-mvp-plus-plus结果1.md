...
第一次失败
![1777115326522](image/test-sop-mvp-plus-plus结果1/1777115326522.png)
第二次给orchestrator发送
```
请先对“内容策划师”发起 ask_user 确认卡，question 必须包含 [[confirm:recruit:内容策划师]]，并提供 yes/no 选项；我回答 yes 后你再调用 recruit_fixed。
然后对“数据分析师”重复同样流程，marker 用 [[confirm:recruit:数据分析师]]。
```
后有弹窗，两个成员分别弹窗询问我是否确认招募，确认后对话记录中有调用，但是左侧列表没有新成员
![1777115513593](image/test-sop-mvp-plus-plus结果1/1777115513593.png)
第3项在首次尝试触发；第1/2项在二次按规范执行后通过。

### 1.2 实测异常记录（已修复）

- **现象**：`recruit_fixed` 成功（日志与对话中有工具调用记录），但左侧成员列表未即时出现新成员。  
- **根因**：`recruit_fixed` 写入 YAML 后，registry 依赖 watchdog 热加载，`agents_updated` 事件发送时机早于 registry 完成 reload，前端拿到旧列表。  
- **修复**：
  1. 后端 `main.py`：当本轮含 `recruit_fixed` / `dismiss_member` 时，发送 `agents_updated` 前延迟 `350ms`。  
  2. 前端 `web/index.html`：收到 `tool_result`（recruit/dismiss）或 `agents_updated` 后，自动调用 `/api/agents` 重试拉取（短间隔多次）以保证最终一致。  
- **验证结果**：汇报后 Cursor 修复代码，重启服务并刷新页面后，成员列表可正常显示新增成员（通过）。  
---
...
![1777116066562](image/test-sop-mvp-plus-plus结果1/1777116066562.png)
---
...
![1777116325831](image/test-sop-mvp-plus-plus结果1/1777116325831.png)
![1777116338190](image/test-sop-mvp-plus-plus结果1/1777116338190.png)
---
...
![1777116628892](image/test-sop-mvp-plus-plus结果1/1777116628892.png)
正确拦截，但是没弹出确认卡。
补充发送以下prompt后有弹窗确认卡，确认解雇后有工具调用，但是左侧列表仍显示队员还在
```
你刚才的 dismiss_member 被拦截了。请立即调用 ask_user，question 必须包含 [[confirm:dismiss:内容策划师]]，并提供 yes/no 选项；我回答 yes 后你再重试 dismiss_member。
```
![1777116730999](image/test-sop-mvp-plus-plus结果1/1777116730999.png)
![1777116947926](image/test-sop-mvp-plus-plus结果1/1777116947926.png)
---
...
上面##5问题上报cursor后帮我改了代码，重启服务器刷新页面后显示原内容策划师已离职，且有系统的心跳提醒
![1777117215073](image/test-sop-mvp-plus-plus结果1/1777117215073.png)
---
...
1、现在21:14了，还是没有。
2、我们在测试开发，已通知cursor把阈值调小：
priority=high 超时阈值：20秒
priority=normal 超时阈值：10秒
心跳扫描周期：5 秒
3、刷新了页面发现有心跳提醒的，但是刷新前确实看不到心跳提醒。
4、数据分析师其实在orchestrator发送任务（19：41：36）后一分钟就回复了（19：42：41），像心跳提醒一样有发但是ui没显示
5、cursor修复上述bug后，我重启服务器刷新页面后最下面有【orchestrator回复中】但没出东西，然后下面不断有系统心跳提示
![1777123974901](image/test-sop-mvp-plus-plus结果1/1777123974901.png)
6、cursor修复上述bug后，我重启服务器刷新页面后，最新的心跳提示中有显示给已离职的内容策划师分配的 task-0001，和给数据分析师分配的 task-0002 ，然而task-0002 之前数据分析师不是已经回复过了吗（19:42:41）

---

## 8. 验证心跳 advisory（需等待或使用 debug 模式）

> ⏱ 当前为 **debug 阈值**：`high=20秒`、`normal=10秒`、`low=120秒`，扫描周期 `5秒`。  
> （生产默认值仍是：`high=10分钟`、`normal=30分钟`、`low=2小时`，扫描周期 `30秒`）  
> **等不了的话** 可以用 §8B 快捷路径。

### 8A 正常等待方式

保持步骤 7 中 `priority=high` 的任务处于 `ready` / `in_progress` 状态，**等待约 20 秒不操作**。  
打开浏览器控制台，查看 WebSocket 帧，应在 5 秒心跳轮询后出现：
...
23:21-23:23 服务端连续触发 heartbeat（silent=1），UI 同步出现 platform 系统提醒，验证通过。

### 8B 快捷路径（直接改数据库）

用 SQLite 浏览器（或命令行）把目标任务的 `updated_at` 改为 2 分钟前（远超 debug 阈值）：
...
| # | DoD 项 | 预期 | 实测结果 | 备注 |
|---|---|---|---|---|
| 1 | 红色拦截裸调被拒 | 返回 marker 教学式错误 | ✅ |dismiss_member/recruit_fixed 未确认时被拦截，返回 confirm marker 指引 |
| 2 | 红色放行（confirm yes） | dismiss/recruit 成功执行 | ✅ |已实测 yes 后成功解雇/招募，且成员列表同步更新 |
| 3 | 心跳 advisory 注入 | 平台发 system_advisory envelope | ✅ |终端有 Heartbeat: ...，聊天区出现 platform 系统提醒（task-0004 等） |
| 4 | 心跳去抖 | 10min 内同任务不重复 | ✅ |当前 debug 窗口按 60s 验证通过；生产口径为 10min |
| 5 | 暂停/恢复生效 | 暂停期间无 advisory | ✅ |Network 看到 POST /pause、/resume 返回 200；恢复后继续有心跳相关日志 |
| 6 | 协调三连不跳步 | send_message→update_task→ask_user→dismiss | ✅ |截图已验证顺序成立，且在 cancel 后未继续 dismiss |
| 7 | A2/C2 hint 正确 | 错误含字段名 + list_tasks 建议 | ✅ |缺参错误返回可读提示，并附 list_tasks(scope='mine') 建议 |
| 8 | C1 无空气泡 | 工具-only 消息无空气泡 | ✅ |	
已修复 system_advisory 导致的空 orchestrator 气泡问题 |
| 9 | A1 文档落盘 | docs/testing-conventions.md 存在 | ✅ |文件存在并已提交 |

本轮采用 debug 心跳参数：interval=5s, high=20s, normal=10s, low=120s, advisory_gap=60s。
生产默认参数：interval=30s, high=10m, normal=30m, low=2h, advisory_gap=10m。
---
