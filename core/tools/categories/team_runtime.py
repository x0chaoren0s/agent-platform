    ...
    **kwargs: Any,
    ...
    for dep in deps:
        ...
        if not dep_str.startswith("task-"):
            return f"错误：depends_on 必须填写 task-id（形如 task-0001），不能填写成员名或其它字符串：{dep_str}。"
        if await store.get(dep) is None:
            return f"错误：依赖任务不存在：{dep}。请先调用 list_tasks(scope='all') 确认上游任务 id。"
    task = Task(
        id="",
        project=_project_name(pdir),
        thread_id=thread_id,
...
async def submit_deliverable(
    ...
    **kwargs: Any,
    ...
    updated = await store.submit_deliverable(task_id, path=deliverable_path, summary=summary, actor=caller_agent)
    if updated is None:
...
async def list_tasks(
    ...
    *,
    ...
    for t in tasks:
        ...
        lines.append(f"| {t.id} | {t.title} | {t.assignee} | {t.status} | {t.priority} | {deps} |")
    return "\n".join(lines)
...
async def give_up(
    ...
    *,
    ...
    updated = await store.update_status(task_id, new_status="failed", actor=caller_agent, note=reason)
    if updated is None:
