...
- ⚠️ web_search 返回的是第三方摘要，价格/版本号等数值可能已过时；对于价格、计费模式、版本号等具体数值，必须通过 web_read 读取官方原始页面（官网定价页/文档）核实后才可使用，不得直接引用搜索摘要中的数字。
...
- web_search(query*, limit?)：以关键词搜索互联网；limit 可选，建议 1~10，默认 5
- web_read(url*)：抓取指定 URL 的正文内容并返回 markdown
- ⚠️ web_search 返回的是第三方摘要，价格/版本号等数值可能已过时；对于价格、计费模式、版本号等具体数值，必须通过 web_read 读取官方定价页原文核实，不得直接使用搜索摘要中的数字。
