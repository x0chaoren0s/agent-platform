...
4. 如果是会议的妙记，应优先使用 [vc +search](skills/lark-vc/references/lark-vc-search.md) 先定位会议，再按需通过 [vc +recording](skills/lark-vc/references/lark-vc-recording.md) 获取 `minute_token`。
5. 会议场景的妙记路由，以及"参与的妙记"如何解释，统一以 [minutes +search](skills/lark-minutes/references/lark-minutes-search.md) 为准。
...
1. 下载妙记音视频文件到本地，或获取有效期 1 天的下载链接。详见 [minutes +download](skills/lark-minutes/references/lark-minutes-download.md)。
...
> **注意**：`+download` 只负责音视频媒体文件。如果用户需要的是逐字稿、总结、待办、章节等纪要内容，请使用 [vc +notes --minute-tokens](skills/lark-vc/references/lark-vc-notes.md)。

### 4. 获取妙记的逐字稿、总结、待办、章节

1. 当用户说"这个妙记的逐字稿""总结""待办""章节"时，**不属于本 skill**。
2. 应使用 [vc +notes --minute-tokens](skills/lark-vc/references/lark-vc-notes.md) 获取对应的纪要产物。
...
> **跨 skill 路由**：逐字稿、AI 总结、待办、章节等纪要内容由 [lark-vc](skills/lark-vc/SKILL.md) 的 `+notes` 命令提供
...
> - 用户只是想看"我的妙记 / 某段时间内的妙记 / 妙记列表"，不要先走 [lark-vc](skills/lark-vc/SKILL.md)，而应直接使用本 skill
> - 用户如果同时提到"会议 / 会 / 开会 / 某场会"，即使也提到了"妙记"，也应优先走 [lark-vc](skills/lark-vc/SKILL.md) 先定位会议，再通过 [vc +recording](skills/lark-vc/references/lark-vc-recording.md) 获取 `minute_token`
> - 用户如果要的是妙记基础信息，拿到 `minute_token` 后用 `minutes minutes get`；用户如果要的是逐字稿、总结、待办、章节，再走 `vc +notes --minute-tokens`
> - “我的妙记”“参与的妙记”等自然语言映射细则，以 [minutes +search](skills/lark-minutes/references/lark-minutes-search.md) 为准
...
> - 用户说"这个妙记的逐字稿 / 总结 / 待办 / 章节" → 使用 [vc +notes --minute-tokens](skills/lark-vc/references/lark-vc-notes.md)
...
| Shortcut                                           | 说明                                                              |
| -------------------------------------------------- | --------------------------------------------------------------- |
| [`+search`](skills/lark-minutes/references/lark-minutes-search.md)     | Search minutes by keyword, owners, participants, and time range |
| [`+download`](skills/lark-minutes/references/lark-minutes-download.md) | Download audio/video media file of a minute                     |

- 使用 `+search` 命令时，必须阅读 [skills/lark-minutes/references/lark-minutes-search.md](skills/lark-minutes/references/lark-minutes-search.md)，了解搜索参数和返回值结构。
- 使用 `+download` 命令时，必须阅读 [skills/lark-minutes/references/lark-minutes-download.md](skills/lark-minutes/references/lark-minutes-download.md)，了解下载参数和返回值结构。
