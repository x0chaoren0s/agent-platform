...
**CRITICAL — 生成任何 XML 之前，MUST 先用 Read 工具读取 [xml-schema-quick-ref.md](skills/lark-slides/references/xml-schema-quick-ref.md)，禁止凭记忆猜测 XML 结构。**

**编辑已有幻灯片页面**：优先用 [`+replace-slide`](skills/lark-slides/references/lark-slides-replace-slide.md)（块级替换/插入，不动页序）；选择 action 和完整读-改-写流程见 [`lark-slides-edit-workflows.md`](skills/lark-slides/references/lark-slides-edit-workflows.md)。
...
也可以分两步（先创建空白 PPT，再逐页添加），详见 [+create 参考文档](skills/lark-slides/references/lark-slides-create.md)。
...
| 文档 | 说明 |
|------|------|
| [xml-schema-quick-ref.md](skills/lark-slides/references/xml-schema-quick-ref.md) | **XML 元素和属性速查，必读** |

### 选读（需要时查阅）

| 场景 | 文档 |
|------|------|
| 需要了解详细 XML 结构 | [xml-format-guide.md](skills/lark-slides/references/xml-format-guide.md) |
| 需要 CLI 调用示例 | [examples.md](skills/lark-slides/references/examples.md) |
...
| 需要编辑已有 PPT 的单个页面 | [lark-slides-edit-workflows.md](skills/lark-slides/references/lark-slides-edit-workflows.md) |
...
可直接复制使用的模板（封面页、内容页、数据卡片页、结尾页）：[slide-templates.md](skills/lark-slides/references/slide-templates.md)
...
| Shortcut | 说明 |
|----------|------|
| [`+create`](skills/lark-slides/references/lark-slides-create.md) | 创建 PPT（可选 `--slides` 一步添加页面，支持 `<img src="@./local.png">` 占位符自动上传），bot 模式自动授权 |
| [`+media-upload`](skills/lark-slides/references/lark-slides-media-upload.md) | 上传本地图片到指定演示文稿，返回 `file_token`（用作 `<img src="...">`），最大 20 MB |
| [`+replace-slide`](skills/lark-slides/references/lark-slides-replace-slide.md) | 对已有幻灯片页面进行块级替换/插入（`block_replace` / `block_insert`），自动注入 id 和 `<content/>`，不改变页序 |
...
| 3350001 | `xml_presentation.slide.replace` 失败（catch-all） | 检查 `block_replace` 替换根是否带 `id=<block_id>`；`<shape>` 是否含 `<content/>`；坐标是否在 960×540 内。详见 [lark-slides-replace-slide.md](skills/lark-slides/references/lark-slides-replace-slide.md) |
...
| 文档 | 说明 |
|------|------|
| [lark-slides-create.md](skills/lark-slides/references/lark-slides-create.md) | **+create Shortcut：创建 PPT（支持 `--slides` 一步添加页面，含 `@` 占位符自动上传图片）** |
| [lark-slides-media-upload.md](skills/lark-slides/references/lark-slides-media-upload.md) | **+media-upload Shortcut：上传本地图片，返回 `file_token`** |
| [lark-slides-replace-slide.md](skills/lark-slides/references/lark-slides-replace-slide.md) | **+replace-slide Shortcut：块级替换/插入，含合法根元素速查与 3350001 排错** |
| [lark-slides-edit-workflows.md](skills/lark-slides/references/lark-slides-edit-workflows.md) | 编辑已有页面的读-改-写流程与 action 决策树 |
| [xml-schema-quick-ref.md](skills/lark-slides/references/xml-schema-quick-ref.md) | **XML Schema 精简速查（必读）** |
| [slide-templates.md](skills/lark-slides/references/slide-templates.md) | 可复制的 Slide XML 模板 |
| [xml-format-guide.md](skills/lark-slides/references/xml-format-guide.md) | XML 详细结构与示例 |
| [examples.md](skills/lark-slides/references/examples.md) | CLI 调用示例 |
...
| [lark-slides-xml-presentations-get.md](skills/lark-slides/references/lark-slides-xml-presentations-get.md) | 读取 PPT 命令详情 |
| [lark-slides-xml-presentation-slide-create.md](skills/lark-slides/references/lark-slides-xml-presentation-slide-create.md) | 添加幻灯片命令详情 |
| [lark-slides-xml-presentation-slide-delete.md](skills/lark-slides/references/lark-slides-xml-presentation-slide-delete.md) | 删除幻灯片命令详情 |
| [lark-slides-xml-presentation-slide-get.md](skills/lark-slides/references/lark-slides-xml-presentation-slide-get.md) | 读取单个幻灯片命令详情 |
| [lark-slides-xml-presentation-slide-replace.md](skills/lark-slides/references/lark-slides-xml-presentation-slide-replace.md) | 原生 slide.replace API 命令详情 |
