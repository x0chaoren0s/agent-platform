...
> **注意**：`+search` 只能查询已结束的历史会议。查询未来的日程安排请使用 [lark-calendar](skills/lark-calendar/SKILL.md)。
>
> **优先级**：当用户搜索历史会议时，应优先使用 `vc +search` 而非 `calendar events search`。calendar 的搜索面向日程，vc 的搜索面向已结束的会议记录，支持按参会人、组织者、会议室等维度过滤。
>
> **路由规则**：如果用户在问“开过的会”“今天开了哪些会”“最近参加过什么会”“已结束的会议”“历史会议记录”，优先使用 `vc +search`。只有在查询未来日程、待开的会、agenda 时才优先使用 [lark-calendar](skills/lark-calendar/SKILL.md)。
>
> **妙记边界**：`+notes` 负责纪要内容、逐字稿和 AI 产物；妙记基础信息请优先看 [`+recording`](skills/lark-vc/references/lark-vc-recording.md) 与 [lark-minutes](skills/lark-minutes/SKILL.md)。
>
...
| Shortcut | 说明 |
|----------|------|
| [`+search`](skills/lark-vc/references/lark-vc-search.md) | Search meeting records (requires at least one filter) |
| [`+notes`](skills/lark-vc/references/lark-vc-notes.md) | Query meeting notes (via meeting-ids, minute-tokens, or calendar-event-ids) |
| [`+recording`](skills/lark-vc/references/lark-vc-recording.md) | Query minute_token from meeting-ids or calendar-event-ids |

- 使用 `+search` 命令时，必须阅读 [skills/lark-vc/references/lark-vc-search.md](skills/lark-vc/references/lark-vc-search.md)，了解搜索参数和返回值结构。
- 使用 `+notes` 命令时，必须阅读 [skills/lark-vc/references/lark-vc-notes.md](skills/lark-vc/references/lark-vc-notes.md)，了解查询参数、产物类型和返回值结构。
- 使用 `+recording` 命令时，必须阅读 [skills/lark-vc/references/lark-vc-recording.md](skills/lark-vc/references/lark-vc-recording.md)，了解查询参数和返回值结构。
...
### minutes（跨域，详见 [lark-minutes](skills/lark-minutes/SKILL.md)）
